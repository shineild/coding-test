...
def solution(nodeinfo):
    answer = [[]]
    nodeinfo = [i + [idx + 1]
                for idx, i in enumerate(nodeinfo)]  # 세 번째 원소로 인덱스 넣기
    # Y 좌표를 기준으로 내림차순 정렬
    nodeinfo = sorted(nodeinfo, key=lambda x: x[1], reverse=True)
    array = []  # 같은 Y 좌표에 여러ㅣ 개의 X 좌표가 들어ㅣ가되록 리스트 구성
    now = -1
    for i in nodeinfo:
        y = i[1]
        if y != now:
